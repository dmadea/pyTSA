#!/usr/bin/env julia
# Run with:  julia -t auto pyTSA/simul/kMC.jl
#
...
#
# Physics (first version)
# -----------------------
# * One CT chromophore at the origin of a 3-D simple-cubic host lattice.
# * No energetic disorder: every host molecule has the same site energy.
# * t = 0: the CT is already excited (CT*).
# * CT* can
#     - decay to the ground state (photon from the centre), rate k_decay = 1/τ
#     - dissociate to a nearest-neighbour polaron pair, Miller–Abrahams rate
#       with a Marcus activation energy
# * The counter-charge stays at the origin; the mobile polaron hops on the
#   host (isoenergetic Marcus / Miller–Abrahams). Hopping onto the origin
#   reforms CT*, which may decay or separate again.
...
#   Rate:               k = ν0 exp(−2 β a − Ea / kT)     (nearest neighbours)
#
# Detailed balance: k_CS / k_CR = exp(−G0 / kT).
...
const BETA_INV_NM = 1.0             # nm⁻¹, inverse localisation length
...
const OUTDIR = joinpath(@__DIR__, "kmc_cube_NN_output")

# six nearest-neighbour displacements on a simple cubic lattice
...
"""Miller–Abrahams rate (s⁻¹) with a Marcus barrier, nearest-neighbour hop."""
function miller_abrahams(ν0, β, a, Ea, T)
    return ν0 * exp(-2 * β * a - Ea / (KB_EV * T))
end
...
function Rates(G0::Float64; λ::Float64 = LAMBDA_EV)
    ...
    k_decay = 1.0 / TAU_CT
    k_cs  = miller_abrahams(NU0, BETA_INV_NM, A_NM, Ea_cs,  T_K)
    k_cr  = miller_abrahams(NU0, BETA_INV_NM, A_NM, Ea_cr,  T_K)
    k_hop = miller_abrahams(NU0, BETA_INV_NM, A_NM, Ea_hop, T_K)
    return Rates(G0, λ, Ea_cs, Ea_cr, Ea_hop, k_decay, k_cs, k_cr, k_hop)
end
...
"""Pick a random neighbour of (x,y,z). If `avoid_origin`, reject (0,0,0)."""
function hop_neighbour(x::Int, y::Int, z::Int, rng, avoid_origin::Bool)
    while true
        d = DIRS[rand(rng, 1:6)]
        nx, ny, nz = x + d[1], y + d[2], z + d[3]
        if avoid_origin && nx == 0 && ny == 0 && nz == 0
            continue
        end
        return nx, ny, nz
    end
...
Returns a `Traj`. Emission time is `t` when `outcome === emitted`.
"""
function simulate_one(r::Rates, rng::AbstractRNG;
    ...
    @inbounds for _ in 1:max_events
        if exciton
            ...
            # charge separation onto a random nearest neighbour
            d = DIRS[rand(rng, 1:6)]
            x, y, z = d[1], d[2], d[3]
            exciton = false
            n_cs += 1
            ...
            at_nn = (x * x + y * y + z * z == 1)
            k_tot = at_nn ? (k_cr + 5.0 * k_hop) : (6.0 * k_hop)
            t += -log(rand(rng)) / k_tot
            if t > t_max
                return Traj(timeout, t_max, n_cs, n_cr, n_hops, r2_max, t_first_cs)
            end
            if at_nn && rand(rng) * k_tot < k_cr
                exciton = true
                x = y = z = 0
                n_cr += 1
            else
                x, y, z = hop_neighbour(x, y, z, rng, at_nn)
                n_hops += 1
                r2 = x * x + y * y + z * z
                r2 > r2_max && (r2_max = r2)
                if r2 > r_esc2
...
struct Ensemble
    ...
    I_all::Vector{Float64}       # emission rate density (1/s), all photons
    I_prompt::Vector{Float64}    # n_cs == 0
    I_delayed::Vector{Float64}   # n_cs ≥ 1
    n_emitted::Int
    n_prompt::Int
    n_delayed::Int
    n_escaped::Int
...
function write_summary_csv(path, ensembles::Vector{Ensemble})
    open(path, "w") do io
        println(io, "G0_eV,lambda_eV,Ea_cs_eV,Ea_cr_eV,Ea_hop_eV,k_decay,k_cs,k_cr,k_hop,k_cs_tot,phi_prompt_theory,phi_em,phi_prompt,phi_delayed,phi_cs,phi_escape,mean_n_cs,mean_n_cr,mean_n_hops,n_emitted,n_prompt,n_delayed,n_escaped,n_timeout")
        for e in ensembles
            r = e.rates
            k_cs_tot = 6 * r.k_cs
            k_exc = r.k_decay + k_cs_tot
            phi_pr_th = r.k_decay / k_exc
            @printf(io,
                "%.5f,%.5f,%.6f,%.6f,%.6f,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%.6e,%d,%d,%d,%d,%d\n",
                r.G0, r.λ, r.Ea_cs, r.Ea_cr, r.Ea_hop,
                r.k_decay, r.k_cs, r.k_cr, r.k_hop, k_cs_tot, phi_pr_th,
...
function print_rates(r::Rates)
    ...
    @printf("    k_decay = %.3e   k_CS (1 NN) = %.3e   6 k_CS = %.3e   k_CR = %.3e   k_hop = %.3e\n",
            r.k_decay, r.k_cs, k_cs_tot, r.k_cr, r.k_hop)
    @printf("    φ_prompt(theory) = k_decay / (k_decay+6k_CS) = %.3e    k_CS/k_CR = %.3e  (exp(-G0/kT)=%.3e)\n",
            r.k_decay / k_exc, r.k_cs / r.k_cr, exp(-r.G0 / (KB_EV * T_K)))
end
...
function main()
    mkpath(OUTDIR)
    println("============================================================")
    println("  kMC hopping around a single CT centre")
    println("============================================================")
    @printf("  λ = %.2f eV    T = %.0f K    τ_CT = %.2e s    a = %.1f nm    β = %.1f nm⁻¹\n",
            LAMBDA_EV, T_K, TAU_CT, A_NM, BETA_INV_NM)
    @printf("  ν0 = %.2e s⁻¹    N_traj = %d    threads = %d    R_escape = %d\n",
            NU0, N_TRAJ, nthreads(), R_ESCAPE)
    println()
