<script module>
    // import { Colormaps } from "../../../tsgraph/src";
...
<script lang="ts">
    import type { Axis, Scale } from "./axis.svelte.js";
    ...
    let { title = $bindable(""), axisAlignment = $bindable("Horizontal"), xAxis, yAxis, zAxis }: SettingsMenuProps = $props()
    ...
    let showZAxis = $derived<boolean>(zAxis !== undefined)
    ...
    // const colormapNames = Colormaps.map(cmap => cmap.name)  // ['asd', 'asd'] //
    // let colormapSelect = $state<string>(fig.colorbar ? fig.colorbar.colormap.name : Colormaps[5].name) // Colormaps[5].name
    // let colormapSelect = $state<string>("asd") // Colormaps[5].name
    ...
    //         fig.colorbar.colormap = Colormaps.filter(cmap => colormapSelect === cmap.name)[0]
    //     }
    ...
    const axisScales: Scale[] = ['Linear', 'Logarithmic', 'Symmetric logarithmic', 'Data bound']
    const axisOrientations: Orientation[] = ['Horizontal', 'Vertical']
         ...
         <div class="input-group sm mb-1">
            ...
            <div class="input-group sm mb-1">
                ...
                <select class="form-select" bind:value={axisAlignment}>
                    {#each axisOrientations as value}
            ...
            {#snippet scaleSelect(axis: Axis)}
                <select class="form-select" bind:value={axis.scale}>
                    {#each axisScales as item}
            ...
            {#snippet snLinthresh(axis: Axis)}
                <Input class="sm" disabled={axis.scale !== "Symmetric logarithmic"} bind:value={axis.symlogLinthresh} />
            ...
            {#snippet snLinscale(axis: Axis)}
                <input type="text" class="form-control" disabled={axis.scale !== "Symmetric logarithmic"} bind:value={axis.symlogLinscale}/>
            {/snippet}
