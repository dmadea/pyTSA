...
export class Matrix extends F32Array {
    ...
    /** Matrix cell write (renamed from `set` to avoid clashing with TypedArray#set). */
    public setCell(value: number, row: number, column: number) {
    ...
    static add(array: Matrix, value: number | F32Array | Matrix | number[]): Matrix {
        const arr = F32Array.add(array, value as number | F32Array | number[]);
    ...
    static mSubtract(mat: Matrix, mat2: Matrix): Matrix {
        ...
        for (let i = 0; i < mat.nrows; i++) {
            for (let j = 0; j < mat.ncols; j++) {
                arr.setCell(mat.get(i, j) - mat2.get(i, j), i, j);
            }
        }
        return arr;
    }
