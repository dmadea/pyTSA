...
const errorMessage: string = "Invalid input, provide a valid number!";
const errors = ref<boolean[]>([false, false]);
  ...
  const submit = () =>  {
    ...
    if (Number.isNaN(parsedData.x)) {
      errors.value[0] = true;
    }
    if (Number.isNaN(parsedData.y)) {
      errors.value[1] = true;
    }
    if (Number.isNaN(parsedData.z)) {
      errors.value[2] = true;
    }
  ...
  <template>
    <CustomModal title="Dimension  multiply" @cancel="emit('cancel')" @submit="submit">
      <div class="input-group input-group-sm" :class="{'mb-3': !errors[0]}">
        ...
        <input type="text" class="form-control" v-model="data.x" placeholder="">
      </div>
      ...
      <div class="input-group input-group-sm" :class="{'mb-3': !errors[1]}">
        ...
        <input type="text" class="form-control" v-model="data.y" placeholder="">
      </div>
      ...
      <div class="input-group input-group-sm" :class="{'mb-3': !errors[2]}">
        ...
        <input type="text" class="form-control" v-model="data.z" placeholder="">
      </div>
