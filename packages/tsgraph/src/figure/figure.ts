...
import { F32Array, NumberArray } from "../../../frontend_svelte/src/lib/array";
...
import { Color, Colormap, Colormaps, ILut } from "../../../frontend_svelte/src/lib/color";
import { HeatMap } from "./heatmap";
import { DraggableLines, Orientation } from "../objects/draggableLines";
import { Axis, AxisType } from "../../../frontend_svelte/src/lib/axis.svelte.ts";
import { ContextMenu } from "../contextmenu";
import { ColorbarContextMenu, FigureContextMenu } from "./figurecontextmenu";
import { Legend } from "../objects/legend";
import { IHeatmapPlot, IThinLinePlot } from "../../../frontend_svelte/src/lib/renderer";
...
export class Colorbar extends Figure {
    ...
    public viewAll(): void {
        if (!this.figure.heatmap) {
            ...
            this.range = {x: rng.x, w: rng.w, y: -1, h: 2};
            this.rangeChanged(this.internalRange);
            return
        }

        const rng = this.range;
        const m = this.figure.heatmap.dataset.data
        const extreme = Math.max(Math.abs(m.min()), Math.abs(m.max()));
