...
import { F32Array } from "../../../frontend_svelte/src/lib/array";
...
interface IGridSettings {
    ...
    widthRatios: F32Array | null,
    heightRatios: F32Array | null
}
...
export class Grid extends GraphicObject {
    ...
    public recalculateGrid(){
        ...
        if (this.gridSettings.widthRatios === null || this.gridSettings.widthRatios.length !== ncols){
            this.gridSettings.widthRatios = new F32Array(ncols).fill(1);  // fill only ones
        }

        if (this.gridSettings.heightRatios === null || this.gridSettings.heightRatios.length !== nrows){
            this.gridSettings.heightRatios = new F32Array(nrows).fill(1);  // fill only ones
        }
