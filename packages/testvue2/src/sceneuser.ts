...
export class SceneUser extends LayoutScene {
  ...
  testAddGrid() {
    ...
    // let n = 1000;
    const x = NumberArray.linspace(-1, 3, n, true);
    const y = NumberArray.random(-1, 3, n, false);
    ...
    const linePlot = this.figx.plotLine(new NumberArray(), new NumberArray());
    const linePlotCol = this.figy.plotLine(
      new NumberArray(),
      new NumberArray()
    );
    ...
    this.dLines.addPositionChangedListener((e) => {
      ...
      if (this.fig?.heatmap) {
        ...
        if (e.yChanged) {
          // get row
          const idxy = this.fig.heatmap.dataset.y.nearestIndex(
            e.realPosition.y
          );
          const row = this.fig.heatmap.dataset.data.getRow(idxy);
          linePlot.y = row;
          this.figx?.replot();
        }

        if (e.xChanged) {
          const idxx = this.fig.heatmap.dataset.x.nearestIndex(
            e.realPosition.x
          );
          const col = this.fig.heatmap.dataset.data.getCol(idxx);
    ...
    const ly = this.figy.addDraggableLines(DraggableLines.Orientation.Vertical);
    const lx = this.figx.addDraggableLines(DraggableLines.Orientation.Vertical);

    ly.addPositionChangedListener((e) => {
      if (this.fig?.heatmap) {
        const idxy = this.fig.heatmap.dataset.y.nearestIndex(e.realPosition.x);
        const row = this.fig.heatmap.dataset.data.getRow(idxy);
    ...
    lx.addPositionChangedListener((e) => {
      if (this.fig?.heatmap) {
        const idxx = this.fig.heatmap.dataset.x.nearestIndex(e.realPosition.x);
        const col = this.fig.heatmap.dataset.data.getCol(idxx);
